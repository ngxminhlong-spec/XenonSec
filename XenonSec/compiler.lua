...
function compileNumericFor(node, ctx)
	...
	emitTempGet(ctx, stepT); emit(ctx, "LOADK", k(ctx, 0)); emit(ctx, "BINOP", k(ctx, ">"))
	local elseJ = emit(ctx, "JMPIFNOT", 0)
	emitTempGet(ctx, counterT); emitTempGet(ctx, limitT); emit(ctx, "BINOP", k(ctx, "<="))
	...
	popScope(ctx)
	local continueHere = here(ctx)
	emitTempGet(ctx, counterT); emitTempGet(ctx, stepT); emit(ctx, "BINOP", k(ctx, "+"))
