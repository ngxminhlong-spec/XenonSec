...
## Banner and header/footer dead code

The old header comment (`Protected by XenonSec :: VM-based Lua
obfuscator (5.1 target)`) was one fixed literal sitting directly in
the template source, so it came out byte-identical in every file this
tool ever produced. It's now built at build time (via `gsub` into a
`__XS_BANNER__` token, the same mechanism already used for the key/
constant-pool/checksum splices) instead of being a hardcoded string in
the template, says only `Protected by XenonSec` — the version/target
descriptor is gone — and its spacing is re-randomized per build, so
the line isn't one constant signature across builds.

`--deadcode <n>` (default `2`, `0` disables) additionally splices `n`
self-contained noise blocks into the header and into the footer. Each
is gated by the logical negation of one of the same number-theoretic
identities the opcode dispatch uses for its opaque predicates — always
false, on any input — so the guarded branch can never execute. Inside,
it only ever declares and arithmetic-combines its own freshly random
local names; it never reads or writes a real local, global, opcode, or
string constant, so it can't change behavior and can't itself leak a
plaintext string. It's pure noise for a static reader to trace through
and rule out.
...
| **Non-fixed banner + dead code** | The header comment is assembled per build via `gsub` rather than a fixed literal (`Protected by XenonSec` only — the version/target text is gone). `--deadcode <n>` (default 2) splices always-false-guarded noise blocks into the header/footer. |
