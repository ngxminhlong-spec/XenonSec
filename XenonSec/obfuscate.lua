...
local function buildBundle(mod, opts)
  ...
  -- 1. Fold every proto's parameter-name strings into the shared
  --    constant pool, replacing them with indices.
  for _, proto in ipairs(mod.protos) do
    local idxs = {}
    for i, pname in ipairs(proto.params) do
      idxs[i] = addConst(mod, pname)
    end
    proto.paramIdx = idxs
  end
  ...
  for _, proto in ipairs(mod.protos) do
    ...
    for i, oldIdx in ipairs(proto.paramIdx) do
      proto.paramIdx[i] = perm[oldIdx]
    end
  end
  ...
  for _, proto in ipairs(mod.protos) do
    ...
    local paramParts = {}
    for _, idx in ipairs(proto.paramIdx) do paramParts[#paramParts + 1] = tostring(idx) end
...
local %OP_LOADK%,%OP_LOADNIL%,%OP_LOADTRUE%,%OP_LOADFALSE%,%OP_GETVAR%,%OP_SETVAR%,%OP_DECLLOCAL%,
      ...
      %OP_ADJUSTMULTI%,%OP_CALL%,%OP_RETURN%
...
local function %run%(%mod%, ...)
  ...
  %runProto% = function(%protoIdx%, %callArgs%, %parentScope%)
    ...
    for %pi% = 1, #%params% do
      ...
      %scope%.vars[%consts%[%params%[%pi%]]] = (%pv% == nil) and %NILV% or %pv%
    end
    local %varargs% = nil
    if %proto%.v then
    ...
    while true do
      ...
      elseif %op% == %OP_NEWTABLE% then
        %push%({}); %ip% = %ip% + 1
      elseif %op% == %OP_GETINDEX% then
        local %key% = %pop%(); local %obj% = %pop%()
        %push%(%obj%[%key%]); %ip% = %ip% + 1
      elseif %op% == %OP_SETINDEX% then
...
local function obfuscate(source, chunkname, opts)
  ...
  local ast = Parser.parse(source, chunkname or "input")
