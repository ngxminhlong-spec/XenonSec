...
--------------------------------------------------------------------
-- Banner: rendered into the output as an inert comment (never
-- executed either way, so this has zero effect on behavior), but
-- filled in via gsub at build time from a token -- __XS_BANNER__ --
-- rather than sitting as one fixed literal inside this template.
-- Says only "Protected by XenonSec"; the old ":: VM-based Lua
-- obfuscator (5.1 target)" descriptor is dropped entirely, and the
-- spacing is rebuilt per build so the comment line isn't one constant
-- byte sequence shared identically by every file this tool produces.
--------------------------------------------------------------------

local function buildBanner()
  local words = { "Protected", "by", "XenonSec" }
  local function pad() return string.rep(" ", 1 + math.random(0, 2)) end
  return pad() .. table.concat(words, pad()) .. pad()
end

--------------------------------------------------------------------
-- Dead code: a self-contained block gated by a genuinely always-false
-- guard (the logical negation of one of the same number-theoretic
-- identities the real opcode dispatch uses -- see OPAQUE_IDENTITIES
-- above), so the branch inside it can never run on any input. It only
-- ever touches its own freshly-named locals with plain arithmetic --
-- no reference to any real local, global, opcode, or string constant
-- -- so it can't change behavior and never becomes a plaintext-string
-- leak of its own. Purely analysis noise: something a static reader
-- has to trace through and rule out by hand.
--------------------------------------------------------------------

local function genFalsePredicate(probeName, depth)
  return "not(" .. genOpaquePredicate(probeName, depth) .. ")"
end

local function buildDeadcodeBlock()
  local n, a, b, c = randomIdent(), randomIdent(), randomIdent(), randomIdent()
  local seed = math.random(2, 999)
  local guard = genFalsePredicate(n, 1 + math.random(0, 1))
  return table.concat({
    "do local ", n, " = ", tostring(seed), " ",
    "if ", guard, " then ",
    "local ", a, ",", b, ",", c, " = ", n, "+1,", n, "*2,", n, "-3 ",
    a, " = (", a, " * ", b, ") + ", c, " ",
    b, " = ", a, " - ", c, " ",
    "end end",
  })
...
local function buildDeadcode(count)
  local parts = {}
  for i = 1, (count or 0) do parts[i] = buildDeadcodeBlock() end
...
local DEFAULTS = {
  ...
  deadcodeBlocks = 2,  -- always-false-guarded noise blocks per header/footer; 0 disables
}
...
local function obfuscate(source, chunkname, opts)
  ...
  header = header:gsub("__XS_BANNER__", (buildBanner():gsub("%%", "%%%%")))
  header = header:gsub("__XS_DEADCODE_HEADER__", (buildDeadcode(opts.deadcodeBlocks):gsub("%%", "%%%%")))
  ...
  footer = footer:gsub("__XS_DEADCODE_FOOTER__", (buildDeadcode(opts.deadcodeBlocks):gsub("%%", "%%%%")))
